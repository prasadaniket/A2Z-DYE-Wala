...
const OurStory = () => {
  ...
  const [currentIndex, setCurrentIndex] = useState(12); // Start at the middle set for infinite loop
  ...
  useEffect(() => {
    // Reset to middle set when reaching boundaries for infinite effect
    if (currentIndex >= artisans.length * 2) {
      setTimeout(() => {
        // Jump back to the same position in set 2 without transition
        setCurrentIndex(artisans.length);
      }, 500); 
    } else if (currentIndex < 0) {
      setTimeout(() => {
        // Jump to the same position in set 2
        setCurrentIndex(artisans.length * 2 - 1);
      }, 500);
    }
  }, [currentIndex, artisans.length]);
      ...
      <section className="team-section-v3">
        <div className="carousel-container">
          ...
          <div className="carousel-nav">
            <button onClick={slideLeft} className="nav-btn">
              <ChevronLeft size={20} />
            </button>
            <button onClick={slideRight} className="nav-btn">
          ...
          <div className="slider-viewport">
            ...
            >
              {[...Array(3)].map((_, setIndex) => 
                artisans.map((artisan, artisanIndex) => (
